  ...
  def call
    tag.h2(class: "text-base font-semibold tracking-tight text-zinc-800 dark:text-zinc-100") do
      if @href
        ...
        link = tag.a(href: @href) do
          safe_join([
            tag.span(class: "absolute -inset-x-4 -inset-y-6 z-20 sm:-inset-x-6 sm:rounded-2xl"),
            tag.span(content, class: "relative z-10")
          ])
        end
        safe_join([overlay, link])
      else
        content
      end
    end
